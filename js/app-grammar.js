...
function ccHtml(c,i){return`<div class="cc"><div class="ccl">${glyphText(c.cl||c.conlang||'')}</div>${c.read||c.kana?'<div class="cck">'+esc(c.read||c.kana)+'</div>':''}<div class="ccj">${esc(c.jp||c.meaning||'')}</div><div style="display:flex;gap:6px;margin-top:8px"><button class="btn btn-xs" data-idx="${i}" onclick="spkC(this)" aria-label="${t('speakAria')}">${icon('speaker')}</button><button class="btn btn-xs btn-r" onclick="delCorpus(${i})" aria-label="${t('delete')}">${icon('x')} ${t('delete')}</button></div></div>`;}
