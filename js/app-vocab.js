...
function wCardHtml(w,idx){return`<div class="wc"><div style="flex:1;min-width:0"><div class="wm">${glyphText(w.word||w.conlang||'')}</div>${w.reading?'<div style="font-size:.7rem;color:var(--ac2)">'+esc(w.reading)+'</div>':''}${w.ipa?'<div style="font-size:.68rem;color:var(--pur);font-family:monospace">'+esc(w.ipa)+'</div>':''}<div class="wmn">${esc(w.meaning||w.jp||'')}</div>${w.tags?.length?'<div style="margin-top:3px">'+w.tags.map(tag=>'<span class="ti">'+esc(tag)+'</span>').join('')+'</div>':''}</div><span class="wpos">${esc(w.pos||'?')}</span><button class="btn btn-gh btn-xs" style="color:var(--rs);border-color:var(--rsb)" onclick="delWord(${idx})" aria-label="${t('deleteAria')}">${icon('x')}</button></div>`;}
